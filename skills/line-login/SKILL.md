---
name: line-login
description: Implements, reviews, and debugs LINE Login v2.1 (OAuth 2.0 + OIDC) — authorization code flow, PKCE (S256), ID Token JWT verification (HS256/ES256), access/refresh token lifecycle, user profile retrieval, friendship/bot linking, and login button design. Use when the user mentions LINE Login, "Log in with LINE", LINE auth, OAuth flow with LINE, ID token verification, token refresh/revocation, bot_prompt, friendship_status_changed, or login button — including auth flows inside LIFF or LINE MINI App.
---
